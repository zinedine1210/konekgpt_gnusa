...
export default function UploadFileKnowledge() {
  ...
  const handlerSubmit = async (e) => {
    ...
    const resultMapMetadata = await UploadFileRepository.mappingMetadata({
      XA: getXA,
    });
    ...
    for (let i = 0; i < data.upload.length; i++) {
      ...
      const resultUploadPerFile = await UploadFileRepository.uploadFile({
        xa: { XA: getXA },
        data: dataForm,
      });
      ...
      const resultSetMetadata = await UploadFileRepository.setMetadata({
        xa: { XA: getXA },
        data: obj,
        uuid: resultUploadPerFile.uuid,
      });
      console.log(resultSetMetadata);
      data._files.push(resultUploadPerFile.uuid);
    }
    ...
    <Layout title={"Create From PDF"} desc={"Halaman untuk membuat word"}>
      <Suspense fallback={"Loading"}>
        <section className="w-full bg-zinc-100 relative h-screen flex">
          <div className="w-full xl:w-1/2 relative h-screen pt-16 overflow-y-auto">
                    ...
                    {data.upload ? (
                      <div className="flex items-center justify-between border p-2">
                        <div className="flex items-center gap-2 w-full">
                          ...
                          <div>
                            <h1 className="text-sm xl:text-base text-zinc-600">
                              {data.upload.length}{" "}
                              {data.upload.length > 1 ? "Files" : "File"}{" "}
                              Selected
                            </h1>
                        ...
                        <button
                          type="button"
                          onClick={() => setData({ ...data, upload: null })}
                          className="w-10 h-10 rounded-md hover:bg-red-100 flex items-center justify-center transition-colors duration-300"
                        >
                      ...
                      <label className="flex justify-center items-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md appearance-none cursor-pointer hover:border-gray-400 focus:outline-none">
                        <div>
                          ...
                          <p className="text-center text-zinc-500 text-xs uppercase tracking-wider font-light">
                            Only PDF (MAX 5MB)
                          </p>
                        </div>
