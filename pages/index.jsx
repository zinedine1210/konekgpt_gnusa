import LastLogin from "@/components/Login/LastLogin";
...
export default function Login() {
  ...
  const getStatus = async (xa) => {
    ...
    if (result?.status == -1 && result?.message == "Token Expired") {
      ...
      localStorage.setItem(
        "auth",
        JSON.stringify({ status: "authentication", data: result })
      );
    }
  };
  ...
  async function tryLogin(payload) {
    try {
      ...
      const requestPromise = AuthRepository.postLogin(
  ...
  return (
    <>
      <Seo title={"Login"} description={"Halaman Login"} />
      <section className="flex items-center justify-center w-full h-screen">
        <div className="">
          <div className="container flex flex-col items-center justify-center min-h-screen px-6 mx-auto">
            <div className="flex justify-center mx-auto">
              {/* <img className="w-auto h-7 sm:h-8" src="https://merakiui.com/images/logo.svg" alt=""/> */}
              <Image src={"/images/logognusa.png"} width={120} height={20} />
            </div>
            ...
            <div className="w-full max-w-md mx-auto mt-6">
              <form onSubmit={(e) => handlerSubmit(e)}>
                ...
                >
                  ...
                  <button
                    className="absolute right-2 bottom-2 inline-block"
                    type="button"
                    onClick={() => handlerHideShow()}
                  >
                    <svg
                      className={`${
                        hide ? "opacity-0 sr-only" : "opacity-100"
                      } transition-all duration-500 w-6 h-6 stroke-zinc-600 dark:stroke-zinc-300`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    ...
                    <svg
                      className={`${
                        hide ? "opacity-100" : "opacity-0 sr-only"
                      } transition-all duration-500 w-6 h-6 stroke-zinc-600 dark:stroke-zinc-300`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                  ...
                  <input
                    ...
                    className="block w-full px-4 py-2 mt-2 text-zinc-700 bg-white border rounded-lg dark:bg-dark dark:text-white dark:border-darkSecondary focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40"
                  />
                  {/* <div>
