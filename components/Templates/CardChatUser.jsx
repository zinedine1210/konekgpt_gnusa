...
import DropdownChatUser from "./DropdownChatUser"
   ...
   return (
        <div className="w-full">
            {index > 0 ? context.chatDetail[index - 1].data.key.fromMe != dataChat.data.key.fromMe ? 
                <h1 className="text-zinc-500 dark:text-zinc-300 text-sm py-1">{dataChat.data.pushName ?? "+"+dataChat.data.key.remoteJid.split("@")[0]}</h1>
            :"" :""}

            {index > 0 ? context.chatDetail[index - 1].data?.participant != dataChat.data?.participant ? 
                <h1 className="text-zinc-500 dark:text-zinc-300 text-sm py-1">{dataChat.data.pushName ?? "+"+dataChat.data?.participant.split("@")[0]}</h1>
            :"" :""}
                    ...
                    <div className="w-fit bg-white dark:bg-darkPrimary pt-1.5 pb-6 px-1.5 shadow-md rounded-md min-w-[80px] max-w-xl relative group">
                        ...
                        }
                        {
                            dataChat.data.message?.protocolMessage ? 
                                <i className="italic text-sm text-zinc-500 dark:text-zinc-300">Pesan ini dihapus</i>
                            :""
                        }
                        {
                            ...
                            <div className='min-w-[300px] max-w-sm relative'>
                                <div className='flex items-center gap-2'>
                                    ...
                                    <div>
                                        ...
                                        <p className='text-zinc-500 dark:text-zinc-300 text-sm'>{dataChat.data.message.documentMessage?.pageCount} Halaman</p>
                                    </div>
                                </div>
                                <button className='absolute top-1/2 -translate-y-1/2 right-2 border hover:bg-zinc-300 transition-all duration-300 border-zinc-300 rounded-full w-8 h-8 flex items-center justify-center'>
                                    <HiDownload className='text-zinc-500 dark:text-zinc-300 font-bold text-lg'/>
                                </button>
                            </div>
                            :""
                        }
