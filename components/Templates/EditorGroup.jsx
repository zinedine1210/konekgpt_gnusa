...
export default function EditorGroup(props) {
    ...
    const handlerSubmit = async e => {
        ...
        const result = await WhatsappRepository.sendGroupMessage({id:context.infoChat.parentId, data:{message:data, instance_key:context.infoChat.parentId, jid:context.infoChat.id.split("@")[0]}})
        console.log(result);
        if(result.success){
        ...
        <div className='flex items-center gap-2'>
            <div ref={dropRef} className='relative'>
                <div className={`${open ? "visible opacity-100 translate-y-0":"invisible opacity-0 translate-y-5"} transition-all duration-300 translate-y-0 absolute bottom-full mb-5 w-full grid grid-cols-1 gap-3`}>
                    <button onClick={() => context.setData({...context, modal:{type:"group", name:"modalsendimage"}})} className='w-full h-auto rounded-md flex items-center justify-center p-2 hover:bg-amber-200 bg-amber-100 shadow-lg'>
                        <IoImage className='text-amber-600 text-2xl'/>
                    </button>
                    <button onClick={() => context.setData({...context, modal:{type:"group", name:"modalsenddocument"}})} className='w-full h-auto rounded-md flex items-center justify-center p-2 hover:bg-lime-200 bg-lime-100 shadow-lg'>
                        <IoDocument className='text-lime-600 text-2xl'/>
                    </button>
                    <button onClick={() => context.setData({...context, modal:{type:"group", name:"modalsendvideo"}})} className='w-full h-auto rounded-md flex items-center justify-center p-2 hover:bg-cyan-200 bg-cyan-100 shadow-lg'>
                        <FaVideo className='text-cyan-600 text-2xl'/>
                    </button>
                    <button onClick={() => context.setData({...context, modal:{type:"group", name:"modalsendcontact"}})} className='w-full h-auto rounded-md flex items-center justify-center p-2 hover:bg-rose-200 bg-rose-100 shadow-lg'>
