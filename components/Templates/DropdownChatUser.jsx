...
import {BsPlusCircle, BsReply} from "react-icons/bs"
...
export default function DropdownChatUser({ dataChat }) {
    ...
    <div ref={dropRef} className="">
        ...
        <div className={`${open ? "not-sr-only opacity-100 scale-100":"opacity-0 scale-0"} z-20 overflow-hidden origin-top-left duration-200 absolute top-0 left-full ml-2 bg-gray-800 rounded-b-2xl rounded-r-2xl shadow-md py-2`}>
            ...
            <button className="text-xs px-5 py-2 hover:bg-gray-900 duration-300 text-start w-full whitespace-nowrap text-white flex items-center gap-2"><IoCopyOutline className="text-base"/> Copy</button>
            <button className="text-xs px-5 py-2 hover:bg-gray-900 duration-300 text-start w-full whitespace-nowrap text-white flex items-center gap-2"><BsReply className="text-base"/> Reply</button>
        </div>
    </div>
  )
}
